...
static double age_estimate[age_size] = {
  0
};
static double lambda[age_size] = {
  0
};

static time_t controlPacketDelay[age_size] = {
  0
};
static time_t controlDepartureTime[age_size] = {
  0
};
...
static double globalBacklog[age_size] = {
  0
};
static time_t backlogTime[age_size] = {
  0
};
...
time_t controlPacketDelay_local[local_size] = {
  0
};
time_t controlDepartureTime_local[local_size] = {
  0
};
time_t backlogTime_local[local_size] = {
  0
};
double globalBacklog_local[local_size] = {
  0
};
...
unsigned char * package(unsigned int seq, string payload) {
  unsigned char * data = (unsigned char * ) malloc((payload.length() + 4) * sizeof(unsigned char));
  ...
  memcpy(&data[4], payload.c_str(), payload.length());
...
void * Receiver(void * sock) {
  ...
  while (1) {
    ...
    if (receiver_seq == 1) { //change it to the first packet recved
      ...
      if (cn == -1) {
        printf("controlAction Thread gone\n");
        exit(1);
      }
    } else {
      mtx.lock();
...
int main(int argc, char * argv[]) {
  ...
  while (1) {
    ...
    if ((numbytes_new = sendto(sockfd, packet, packet_size, 0,
        p -> ai_addr, p -> ai_addrlen)) == -1) {
      printf("talker2: sendto");
      exit(1);
    }
